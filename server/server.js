...
function getOrCreateRoom(roomId) {
  if (!rooms.has(roomId)) {
    rooms.set(roomId, {
      ...
      handRaised: new Set(),
      chatMessages: [] // Store chat messages for new joiners
    })
  }
...
io.on("connection", socket => {
  ...
  socket.on("join-room", ({ roomId, userId, userName, password }) => {
    ...
    try {
      ...
      room.members.set(userId, {
        ...
        handRaised: room.handRaised.has(userId)
      })

      console.log(
        `User joined room ${roomId}: ${userId}, Total members: ${room.members.size}`
      )
  ...
  // Chat messages with file upload - FIXED DUPLICATES
  socket.on("chat:send", ({ roomId, message }) => {
    ...
    try {
      // Rate limiting
      if (
        !socket.data.lastChatTime ||
        Date.now() - socket.data.lastChatTime > 1000
      ) {
        // Add unique ID to prevent duplicates
        message.id = `${socket.data.userId}-${Date.now()}-${Math.random()
          .toString(36)
          .substr(2, 9)}`

        // Store message in room for new joiners
        const room = getOrCreateRoom(roomId)
        ...
        // Broadcast to ALL users including sender (let clients handle isMe)
  ...
  socket.on("whiteboard:enable", roomId => {
    ...
    if (room.hostId === socket.data.userId) {
      console.log(
        `Whiteboard enabled for room ${roomId} by host ${socket.data.userId}`
      )
  ...
  socket.on("whiteboard:disable", roomId => {
    ...
    if (room.hostId === socket.data.userId) {
      console.log(
        `Whiteboard disabled for room ${roomId} by host ${socket.data.userId}`
      )
  ...
  socket.on("whiteboard:drawing", ({ roomId, drawing }) => {
    if (!socket.data?.userId || !socket.data?.roomId) return
    ...
    if (room.allowedUsers.has(socket.data.userId)) {
      // Add unique ID to prevent duplicates
      drawing.id = `${socket.data.userId}-${Date.now()}-${Math.random()
        .toString(36)
        .substr(2, 9)}`
  ...
  socket.on("whiteboard:background", ({ roomId, background }) => {
    if (!socket.data?.userId || !socket.data?.roomId) return
  ...
  socket.on("whiteboard:clear", roomId => {
    ...
    if (room.allowedUsers.has(socket.data.userId)) {
      ...
      console.log(
        `Whiteboard cleared for room ${roomId} by user ${socket.data.userId}`
      )
    }
  })

  // Allow user to draw
  socket.on("whiteboard:allow-user", ({ roomId, userId, allow }) => {
    if (!socket.data?.userId || !socket.data?.roomId) return
    ...
    if (room.hostId === socket.data.userId) {
      ...
      io.to(roomId).emit("whiteboard:user-permission", {
        userId,
        canDraw: allow
      })
      ...
      console.log(
        ...
        } to draw in room ${roomId}`
      )
    }
  })

  // Make user host
  socket.on("host:make-host", ({ roomId, userId }) => {
    if (!socket.data?.userId || !socket.data?.roomId) return
  ...
  socket.on("hand:lower-all", roomId => {
    if (!socket.data?.userId || !socket.data?.roomId) return
  ...
  // Moderator controls
  socket.on("moderator:mute-user", ({ roomId, userId }) => {
    if (!socket.data?.userId || !socket.data?.roomId) return
  ...
  socket.on("moderator:remove-user", ({ roomId, userId }) => {
    if (!socket.data?.userId || !socket.data?.roomId) return
  ...
  socket.on("reaction:send", ({ roomId, reaction }) => {
    if (!socket.data?.userId || !socket.data?.roomId) return
    // Add unique ID to prevent duplicates
    reaction.id = `${socket.data.userId}-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`
  ...
  socket.on("disconnect", reason => {
    ...
    if (socket.data?.roomId && socket.data?.userId) {
      ...
      if (rooms.has(roomId)) {
        ...
        console.log(
          `User left room ${roomId}: ${userId}, Remaining members: ${room.members.size}`
        )
        ...
        if (room.hostId === userId) {
          ...
          room.hostId =
            remainingMembers.length > 0 ? remainingMembers[0].id : null
